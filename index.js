...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'ORG: Pengembang XBot;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=16999469735:+16 999469735\n' 
            + 'END:VCARD' 
prefix = '$'
blocked = []          
...
client.on('group-participants-update', async (anu) => {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["5516999469735@s.whatsapp.net"] 
          ...
          case 'ping':
            ...
            case 'kangcopet':
                ...
                case 'hidetag':
					...
					if (!isGroupAdmins) return reply('QUEM E VC?')
			...
			case 'loli': 
               ...
               if (!isGroup) return reply(mess.only.group)
                ...
                break						
				case 'add':
					if (!isOwner) return reply('Não sera possível usar')
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
            ...
            case 'owner':
            case 'creator':
                  client.sendMessage(from, {displayname: "Albion Desenvolvedor", vcard: vcard}, MessageType.contact, { quoted: mek})
       client.sendMessage(from, 'Grupo albion Desenvolvedor',MessageType.text, { quoted: mek} )
           break    
           case 'setname':
                if (!isGroup) return reply(mess.only.group)
			    if (!isGroupAdmins) return reply(mess.only.admin)
           ...
           case 'demote':
				...
				case 'promote':
					...
					break	
			     	case 'kick':
					if (!isOwner) return reply('comando Bloqueado')
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
				...
				case 'toimg':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Terminei'})
						fs.unlinkSync(ran)
					})
					break
